...
- "列出我的未关闭服务请求"  
  `mes -o json sr list --status 1 --person-id <uid>`
- "搜索标题包含 'Oracle' 的服务请求"  
  `mes -o json sr list Oracle`  
  （等效于 `mes -o json sr list --title "Oracle"`）
- "看服务请求 123 详情，并列出附件列表"  
  `mes sr view 123`
- "用 JSON 看服务请求 123 详情（含完整附件信息）"  
  `mes -o json sr view 123`
- "给服务请求 123 回复：已处理"  
  `mes sr reply 123 --text "已处理，等待验证" --dry-run`
- "给服务请求 123 回复 Markdown 格式内容"  
  `mes sr reply 123 --markdown "## 处理结果\n\n问题已解决，详见以下步骤：\n\n1. 检查配置\n2. 重启服务\n3. 验证功能" --dry-run`

- "服务请求 123 附件太多，我先看看有哪些"  
  `mes sr view 123`  
  （输出会显示附件列表：`[1] 日志文件.log (pdf, 2.3MB)  id=6035`）
- "下载服务请求 123 的指定附件（用 view 中显示的 id）"  
  `mes sr download 123 --file-id 6035`
- "下载服务请求 123 的全部附件（默认行为）"  
  `mes sr download 123`
- "客户上传了报错截图，我下载下来分析"  
  `mes sr download 123 --file-id 6035 -O ~/Downloads/客户日志.pdf`
- "下载到指定目录"  
  `mes sr download 123 -O ~/Downloads/`

- "我回复时附上本地排查截图"  
  `mes sr reply 123 --text "排查截图见附件" --file /home/user/error.png --dry-run`  
  提交时去掉 `--dry-run`。
- "故障已处理，回复客户并附上服务报告 PDF"  
  `mes sr reply 123 --text "故障已处理完毕，详见附件服务报告" --file ./服务报告.pdf`
- "我需要内部回复（不让客户看到附件），附上内部分析文档"  
  `mes sr reply 123 --text "内部分析：根因是 XXX" --file ./分析.doc --internal`
- "使用 Markdown 格式回复，包含代码块和格式化内容"  
  `mes sr reply 123 --markdown "## 问题分析\n\n根据日志分析，问题原因如下：\n\n\`\`\`sql\nSELECT * FROM users WHERE id = 123;\n\`\`\`\n\n**解决方案**：优化索引" --dry-run`
...
- "找到 '云和恩墨虚拟客户' 公司的合同子项"
  ...
  **Mappings & Heuristics**:
  ...
  - **Items List**: `itemId` -> `--acc-id` (used in `sr create`)
...
1. 先读取服务请求详情（`mes sr view <id>`），提炼状态、进度、关键动作。
...
`mes sr reply 5184 --text "xxxx" --dry-run`

确认后提交（去掉 `--dry-run`）：

`mes sr reply 5184 --text "xxxx"`

**使用 Markdown 格式回复**

`mes sr reply 5184 --markdown "## 处理进展\n\n已完成初步排查，详见以下分析：\n\n- 问题根因：配置错误\n- 解决方案：更新配置文件\n- 当前状态：等待验证" --dry-run`
