---
name: mermaid-share
description: Render Mermaid code to an SVG and upload it to S.EE to return shareable links. Use this whenever the user gives Mermaid diagram code and wants a hosted SVG URL, image link, Markdown embed, or asks to render then upload/share a diagram in one flow.
version: 1.0.0
user-invocable: true
metadata:
  openclaw:
...
Turn Mermaid code into a hosted SVG by orchestrating the sibling `mermaid-viz` and `see-uploader` skills.
...
- wants to compare themes and publish one of the rendered SVGs
...
2. Choose a theme. Default to `zinc-dark` unless the user asks for another supported theme.
...
6. If the type is supported, use the sibling `mermaid-viz` skill to render the Mermaid code to an SVG file.
7. Use the sibling `see-uploader` skill to upload that SVG file to S.EE.
...
- invoke `mermaid-viz` to produce an SVG only after the type check passes
- pass the generated SVG file path to `see-uploader`
...
- Use SVG output only; this workflow does not produce PNG.
