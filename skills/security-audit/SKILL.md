...
### Dynamic companion selection (Token & Cost Optimization)

Never load or inject all 10 domain companion files into context at once. `SKILL.md` is the single lightweight entry point.

During **Phase 1 (Reconnaissance)**:
1. Identify the target repository's technology stack, architecture, and trust boundaries (e.g. Go backend + Next.js frontend, Python FastAPI, C/C++ native binary).
2. Dynamically select **ONLY** the domain companions required by active, source-visible boundaries:
   - Web application / REST / WebSocket APIs / AI Agents: `WEB-PROTOCOL-AND-AUTH.md`, `AI-AND-LLM.md`, `DATA-ISOLATION-AND-LIFECYCLE.md`, `SUPPLY-CHAIN-AND-RELEASE.md`.
   - Web frontends: `CLIENT-SIDE.md`.
   - Microservices & message queues: `PROTOCOLS-RPC-AND-MESSAGING.md`, `RESOURCE-EXHAUSTION-AND-AVAILABILITY.md`.
   - Cloud / Infrastructure-as-Code: `CLOUD-AND-DEPLOYMENT.md`.
3. **Explicitly skip and exclude irrelevant companions** (e.g., skip `MEMORY-SAFETY-AND-BINARY.md` if there is no C/C++/Rust manual memory management; skip `DESKTOP-MOBILE-AND-LOCAL-IPC.md` if there is no desktop/mobile app).

This ensures lean context window usage, avoids cognitive distraction, and reduces token overhead by 70%+.
...
4. **Structured output** — write all final `confirmed`, `needs_validation`, and `rejected` records to `findings.json`; validate it with `resources/report-schema.json` and `scripts/validate-findings.cjs`, and validate the coverage claim with `scripts/validate-coverage-ledger.cjs`.
