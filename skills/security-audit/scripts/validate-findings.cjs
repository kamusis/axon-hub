...
function run(file) {
  ...
  try {
    const candidatePaths = [
      path.join(__dirname, "../resources/report-schema.json"),
      path.join(__dirname, "resources/report-schema.json"),
      path.join(__dirname, "report-schema.json"),
    ];
    const schemaPath = candidatePaths.find((p) => fs.existsSync(p)) || candidatePaths[0];
