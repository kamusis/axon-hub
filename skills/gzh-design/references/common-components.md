...
要点（**关键，避免大段空白与错位**）：① 顶栏三色圆点 + 语言名（无语言可删该 span）；② **每行代码用一个 `<p style="margin:0;...">`，不要用 `white-space:pre`**——否则 HTML 源码里 span 前的缩进和行间换行会被原样渲染成大左缩进 + 空行；③ 需要缩进时在 span 文字里用全角空格 `　`（不要靠源码空格）；④ 行距只靠 `line-height:1.6` 控制，padding 用 `11px 14px`，保持紧凑；⑤ 长行会自动换行，不溢出；⑥ **中文注释一律用行上方注释**：严禁在代码行尾通过连续空格手动对齐中文注释（中英文在不同终端等宽字体下宽度不同，会导致严重错位）；⑦ **禁止把带 `┌─┐│` 的 ASCII 框图塞入代码块**：ASCII 拓扑图在公众号中必须转为原生自适应 Flex 结构卡片流或 SVG 矢量流程图。
