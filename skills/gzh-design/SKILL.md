...
7. **尾部作者签名区（仅末尾一处）**：
   - 优先从文章 Frontmatter 的 `author` 字段提取真实作者名（如 `author: Mopheus Team` / `author: Kamus`）；用户在请求/偏好里给了署名或简介就直接填入；
   - 原文末尾已有作者签名段（如"我是 XXX…"）则直接沿用原文署名；
   - 仅在上述信息皆无时，保留 `{{作者名}}` / `{{简介}}` 占位，并在交付时提示用户替换。
...
10. **架构图与 ASCII 框图自适应转换（关键）**：原文中带有 `┌─┐│` 的复杂 ASCII 框图或拓扑图，**严禁直接原样塞入代码块**（Unicode 制表符在移动端与各浏览器引擎中非严格等宽且窄屏会强行折行散架）。必须自动转换为当前主题风格的 **自适应 HTML 结构卡片流（Flexbox 边框盒与层级流）**，或 **SVG 矢量流程图（如组件 23）**，确保在手机和 PC 上 100% 像素级对齐。
11. **代码块中英混排注释规范（行上注释）**：代码块中禁止在代码行尾通过连续空格手动对齐中文注释（中英字符宽度在不同字体下无法对齐）。一律采用 **标准行上方注释（Above-Line Comment）**：将中文注释写在代码行的前一行（如 `// 注释说明\nFieldType`），保证各端展示整洁对齐。
...
- **ASCII 框图禁入代码块**：Unicode 框线符号在移动端和不同浏览器引擎中属于 Fallback 字体，与空格非等宽且窄屏强行折行会导致 100% 错位。严禁将带 `┌─┐│` 的复杂 ASCII 框图塞入代码块，必须转为主题自适应的 Flexbox 结构卡片流或 SVG 流程图。
- **代码块注释禁止行尾空格对齐**：中英混排在不同等宽字体下列宽不一致。一律使用标准行上方注释（Above-Line Comment）。
